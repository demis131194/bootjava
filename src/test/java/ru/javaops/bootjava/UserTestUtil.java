...
public class UserTestUtil {
    ...
    public static void assertNoIdEquals(List<User> actual, List<User> expected) {
        assertThat(actual).usingRecursiveComparison().ignoringFields("id", "password").isEqualTo(expected);
    }
    ...
    public static List<User> asUserList(MvcResult mvcResult) throws UnsupportedEncodingException, IOException {
        String jsonActual = mvcResult.getResponse().getContentAsString();
        return JsonUtil.readValuesFromHal(jsonActual, User.class, "users");
    }
    ...
    public static ResultMatcher jsonMatcher(List<User> expected, BiConsumer<List<User>, List<User>> equalsAssertion) {
        return mvcResult -> equalsAssertion.accept(asUserList(mvcResult), expected);
    }
}
